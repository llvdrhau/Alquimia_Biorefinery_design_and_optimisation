...
%% make sure you reference the correct working directory 
currentDirectory = cd();
pos = strfind(currentDirectory, 'Kinetic model MCF v4 (updated)');
pos = pos + length('Kinetic model MCF v4 (updated)');
workingDirectiory = strcat(currentDirectory(1:pos), 'Stoichiometric model\R Generation\');

flagCase.computer=workingDirectiory;
...
for i = 1:N
    ...
    pH = pHRange(1) + (pHRange(2)-pHRange(1)).*rand();
