...
splitRatio = 0.5; % ratio concentration S2/S1 [0:1] In this case always 50/50 (change for validation set, maybe even randomise?¿) 
flagCase.substrates_ratio = splitRatio; % ratio concentration S2/S1 [0:1] In this case always 50/50 (change for validation set)

rows = size(combineSubtrateFlags,1)*length(pHRange); 
colInputMatrix = 23;
colOutputMatrix = 8;  
...
for i = 1:size(combineSubtrateFlags,1)
    for j = 1:length(pHRange)
        %% select substrate and pH
        flagCase.substrate1 = 1; % choose substrate 1
        flagCase.substrate2 = 1; % choose substrate 2
        pH = pHRange(j);
        ...
        inputMatrix(rowCounter+j,:) = inputs; 
        ...
        %% 
        % 2nd part is not necesary, forget dissosation for a second, not
        % important here
...
%         outputMatrix(rowCounter+j,:) = allOutputYield;
...
%%
save('Data_4_ML_model','nameOutputs', 'nameInputs','outputMatrix', 'inputMatrix');
